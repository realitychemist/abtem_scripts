...
# TODO: This is probably fundamentally the wrong way to approach this problem
...
divergences = _minmax_norm(np.array([0 if div is None else div
                                     for row in divergences for div in row])
                           ).reshape(tensor_data.shape[0], tensor_data.shape[1]).astype("float32")

# %%
background = np.mean(divergences[:100, :100])
noise_std = np.std(divergences[:100, :100])
lowthresh, highthresh = background-noise_std, background+noise_std
plt.imshow(divergences, cmap="binary", vmin=lowthresh, vmax=highthresh)
